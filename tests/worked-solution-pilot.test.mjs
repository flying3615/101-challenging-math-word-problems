...
const containsNumberToken = (text, token) => new RegExp(`(?<!\\d)${escapeRegExp(token)}(?!\\d|\\.\\d)`).test(text);

assert.equal(containsNumberToken('15 and 50', '5'), false, 'Number-token matching must not treat 5 as a match inside 15 or 50.');
assert.equal(containsNumberToken('$5.40', '5'), false, 'Number-token matching must not treat 5 as a match inside a decimal amount.');
assert.equal(containsNumberToken('$5', '5'), true, 'Number-token matching must accept a standalone monetary number.');
...
for (const id of expectedIds) {
  ...
  for (const language of ['en', 'zh-Hans']) {
    ...
    assert.ok(questionNumberTokens.some(token => containsNumberToken(solutionText, token)), `Q${id}'s ${language} solution must use a question-specific number.`);
    ...
    for (const token of answerNumberTokens) assert.ok(containsNumberToken(solutionText, token), `Q${id}'s ${language} solution must show the answer number ${token}.`);
  }
}
