...
assert.deepEqual(bank.filter(question => question.solution).map(question => question.id), expectedIds, 'Every Book 5 question should include a worked solution.');
...
assert.match(page, /id="solutionButton"/, 'The standard Book 5 practice page must expose a guided-solution control.');
assert.match(page, /id="hideSolution"/, 'The standard Book 5 practice page must let learners hide an open solution.');
assert.match(page, /solutionStages/, 'Book 5 solutions must reveal progressively.');
assert.match(i18n, /startGuidedSolution/, 'The shared bilingual controller must label guided solutions.');

console.log('PASS: all 101 Book 5 questions provide progressive bilingual worked solutions.');
