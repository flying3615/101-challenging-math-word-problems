(() => {
  ...
  const text = {
    en: {
      ...
      storyMap: 'QUESTION {{id}} STORY MAP', storyMapTitle: 'Story map: {{title}}', keyValues: 'Key values from this question', noValues: 'Read the quantities in the story', whole: 'of 1 whole', equalParts: '{{count}} equal parts', findPart: 'Find the part or remainder the question asks for', moneyValue: 'Money value {{index}}', fromQuestion: 'from this question', nextStep: 'Next step', equalSets: 'equal sets', matchGroups: 'match each value to its group', knownMeasure: 'Known measure', find: 'Find', knownValue: 'Known value', compareWith: 'Compare with', target: 'Question target', given: 'given in the story', identify: 'Identify this relationship',
    ...
    'zh-Hans': {
      ...
      storyMap: '第 {{id}} 题数量关系图', storyMapTitle: '数量关系图：{{title}}', keyValues: '本题的关键数量', noValues: '阅读题目中的数量', whole: '个整体', equalParts: '{{count}} 个相等部分', findPart: '找出题目要求的部分或余数', moneyValue: '金额 {{index}}', fromQuestion: '来自本题', nextStep: '下一步', equalSets: '相等的组', matchGroups: '把每个数值与题中的组对应起来', knownMeasure: '已知量', find: '求', knownValue: '已知数量', compareWith: '比较数量', target: '所求数量', given: '题目中已知', identify: '确定这个数量关系',
  ...
  function applyHomeChrome() {
    ...
    setNodeText('.masthead p', t('homeLead')); setNodeText('.collection-note', t('homeNote')); setNodeText('#books-title', t('chooseBook')); setNodeText('.footer', t('footer'));
    document.querySelectorAll('.book').forEach(card => {
      const number = card.dataset.book; setNodeText('.book-no', isChinese() ? '101 道挑战数学应用题' : '101 Challenging Math Word Problems', card); setNodeText('h2', isChinese() ? `第 ${number} 册` : `Book ${number}`, card); setNodeText('p', t('readyDescription'), card); setNodeText('.tag', t('practiceReady'), card);
